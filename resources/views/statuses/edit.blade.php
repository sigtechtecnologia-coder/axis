        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('statuses.update', $status) }}" class="space-y-6">
                        @csrf
                        @method('PUT')

                        @include('statuses._form', ['status' => $status, 'contexts' => $contexts])

                        <div class="flex items-center gap-3">
                            <button class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700">
                                Salvar
                            </button>
                            <a href="{{ route('statuses.index', ['context' => $status->context]) }}"
